...
void TcpClient::recvMsg() {
    ...
    switch (pdu->uiMsgType) {
        ...
        case ENUM_MSG_TYPE_UPLOAD_INIT_RESPOND: {
            ...
            // 获取服务器返回的状态信息
            QString statusInfo = QString(pdu->caData);

            // 检查是否初始化失败
            if (statusInfo == "INIT_FAIL") {
                QMessageBox::warning(nullptr, "上传失败",
                                     "服务器初始化上传任务失败");
                break;
            }

            // 解析服务器返回的分片状态信息
            // 格式：chunkIndex1,status1;chunkIndex2,status2;...
            QStringList chunks = statusInfo.split(';', Qt::SkipEmptyParts);

            for (const QString& chunk : chunks) {
                QStringList parts = chunk.split(',');
                if (parts.size() == 2) {
                    qint64 chunkIndex = parts[0].toLongLong();
                    bool isCompleted = (parts[1] == "1");

                    if (isCompleted) {
                        pBook->m_uploadedChunks.append(chunkIndex);
                    }
                }
            }
