...
void TcpClient::downloadFilePre(PDU* pdu) {
	...
	if (strlen(caFileName) > 0 &&
		...
		OpeWidget::getInstance().getBook()->setDownloadStatus(
			true);  // 标记开始下载文件
...
void TcpClient::recvMsg() {
	...
	case ENUM_MSG_TYPE_UPLOAD_INIT_RESPOND: {
		qDebug() << "receive msg type: ENUM_MSG_TYPE_UPLOAD_INIT_RESPOND 上传初始化";
		...
		for (const QString& chunk : chunks) {
			...
			if (parts.size() == 2) {
				...
				qint64 chunkIndex = parts[0].toLongLong(&ok1);  // 解析分片索引
				int status = parts[1].toInt(&ok2);             // 解析分片状态

				if (ok1 && ok2) {
					qDebug() << "Chunk" << chunkIndex << "status:" << status;
					if (status == 1) {
						pBook->m_uploadedChunks.append(chunkIndex);  // 如果状态为1，添加到已上传分片列表
					}
				}
				else {
	...
	case ENUM_MSG_TYPE_UPLOAD_CHUNK_RESPOND: {
		...
		std::cout << "chunkIndex:" << chunkIndex << ' ' << "result: " << result << std::endl;
