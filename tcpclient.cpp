...
TcpClient::TcpClient(QWidget* parent) : QWidget(parent), ui(new Ui::TcpClient) {
    ...
    for (int i = 0; i < 0; ++i) {
        ...
        // 线程完成时，删除工作对象和线程
        connect(worker, &TcpSocketWorker::finished, worker,
                &QObject::deleteLater);
    ...
    m_downloadDialog = new DownloadProgressDialog(this);
    ...
    //		this->func1(i);
    //		});*/
    //	connect(sockets[i], &QTcpSocket::errorOccurred, this, [this,
    // i](QAbstractSocket::SocketError error) { 		qDebug() <<
    // "Socket" << i << "error:" << error;
    //		});
    // }
}
...
TcpClient::~TcpClient() {
    ...
    m_tcpSocket.close();
    std::cout
        << "TcpClient::~TcpClient() called myTcpSocket.close() and delete ui"
        << std::endl;
    delete ui;
}
...
void TcpClient::setCurPath(QString strPath) { m_strCurPath = strPath; }

void TcpClient::showConnect() {
    QMessageBox::information(this, "成功", "TCP链接服务端成功");
    qDebug() << TcpClient::getTcpSocket().localAddress() << ' '
             << TcpClient::getTcpSocket().localPort() << "\n";
}
...
void TcpClient::addFriendRequest(PDU* pdu) {
    ...
    int ret = QMessageBox::information(
        this, "添加好友", QString("%1 want to add you as friend.").arg(caName),
...
void TcpClient::addFriendAgree(PDU* pdu) {
    ...
    QMessageBox::information(
        this, "添加好友",
        QString("添加好友成功！%1 同意了您的好友添加请求").arg(friendName));
}
...
void TcpClient::addFriendFefuse(PDU* pdu) {
    ...
    QMessageBox::information(
        this, "添加好友",
        QString("添加好友失败！%1 拒绝了您的好友添加请求").arg(friendName));
}
...
void TcpClient::deleteFriend(PDU* pdu) {
    ...
    QMessageBox::information(this, "删除好友",
                             QString("%1 已经删除了您的好友").arg(loginName));
}

void TcpClient::privateChat(PDU* pdu) {
    if (PrivateChat::getInstace().isHidden()) {
        PrivateChat::getInstace().setWindowTitle("cur user:" +
                                                 QString(pdu->caData));
...
void TcpClient::delDir(PDU* pdu) {
    ...
    OpeWidget::getInstance().getBook()->returnPre();
}
...
void TcpClient::downloadFilePre(PDU* pdu) {
    ...
    if (strlen(caFileName) > 0 &&
        ...
        if (!OpeWidget::getInstance().getBook()->m_pFile.open(
                QIODevice::WriteOnly)) {
            QMessageBox::warning(this, "下载文件",
                                 "下载文件失败：本地文件无法操作");
            return;
        }
...
void TcpClient::recvMsg() {
    if (OpeWidget::getInstance().getBook()->getDownloadStatus()) {
        ...
        // 更新下载进度
        m_downloadDialog->updateProgress(pBook->m_iRevice, pBook->m_iTotal);
        ...
        if (pBook->m_iTotal == pBook->m_iRevice) {
            ...
            pBook->setDownloadStatus(false);
            m_downloadDialog->hide();  // 隐藏进度对话框
            QMessageBox::critical(this, "下载文件",
                                  "下载文件失败：传输的文件错误");
        }
        return;
    }
    ...
    switch (pdu->uiMsgType) {
        case ENUM_MSG_TYPE_REGIST_RESPOND: {
            regist(pdu);
            qDebug()
                << "receive msg type: ENUM_MSG_TYPE_REGIST_RESPOND 注册成功";
            break;
        }
        case ENUM_MSG_TYPE_LOGIN_RESPOND: {
            login(pdu);
            qDebug()
                << "receive msg type: ENUM_MSG_TYPE_LOGIN_RESPOND 登录成功";
            break;
        }
        ...
        case ENUM_MSG_TYPE_DEL_DIR_RESPOND: {
            delDir(pdu);
            qDebug()
                << "receive msg type: ENUM_MSG_TYPE_DEL_DIR_RESPOND 删除文件夹";
            break;
        }
        ...
        case ENUM_MSG_TYPE_DEL_FILE_RESPOND: {
            ...
            qDebug()
                << "receive msg type: ENUM_MSG_TYPE_DEL_FILE_RESPOND 删除文件";
            break;
        }
        ...
        case ENUM_MSG_TYPE_SHARE_FILE_NOTE: {
            shareFileNote(pdu);
            qDebug()
                << "receive msg type: ENUM_MSG_TYPE_SHARE_FILE_NOTE 分享文件";
            break;
        }
        case ENUM_MSG_TYPE_UPLOAD_INIT_RESPOND: {
            qDebug() << "receive msg type: ENUM_MSG_TYPE_UPLOAD_INIT_RESPOND "
                        "上传初始化";
            ...
            // 清空已上传分片列表
            pBook->m_uploadedChunks.clear();
            qDebug() << pdu->caData;
            std::cout << "pdu->uiMsgLen:" << pdu->uiMsgLen << std::endl;
            qDebug() << "pdu->msg:" << (char*)pdu->caMsg;

            // 将 (char*)pdu->caMsg 转换为 QString
            QString statusInfo = QString::fromUtf8((char*)pdu->caMsg);

            // 按分号分割字符串
            QStringList chunks = statusInfo.split(';', Qt::SkipEmptyParts);
            qDebug() << "Parsed chunks:" << chunks;

            // 遍历每个分片信息
            for (const QString& chunk : chunks) {
                QStringList parts = chunk.split(',');
                if (parts.size() == 2) {
                    bool ok1 = false, ok2 = false;
                    qint64 chunkIndex =
                        parts[0].toLongLong(&ok1);      // 解析分片索引
                    int status = parts[1].toInt(&ok2);  // 解析分片状态

                    if (ok1 && ok2) {
                        qDebug()
                            << "Chunk" << chunkIndex << "status:" << status;
                        if (status == 1) {
                            pBook->m_uploadedChunks.append(
                                chunkIndex);  // 如果状态为1，添加到已上传分片列表
                        }
                    } else {
                        qWarning() << "Invalid chunk format:" << chunk;
                    }
                } else {
                    qWarning() << "Invalid chunk format:" << chunk;
                }
            ...
            // 创建上传队列
            pBook->createUploadQueue();

            // 开始处理上传队列
            pBook->processUploadQueue();
            break;
        }
        case ENUM_MSG_TYPE_UPLOAD_CHUNK_RESPOND: {
            qDebug() << "receive msg type: ENUM_MSG_TYPE_UPLOAD_CHUNK_RESPOND "
                        "分片上传响应";
            ...
            // 解析服务器返回的分片上传结果
            // 格式：分片索引 上传结果(0成功,1失败)
            qint64 chunkIndex;
            int result;
            sscanf(pdu->caData, "%lld %d", &chunkIndex, &result);
            std::cout << "chunkIndex:" << chunkIndex << ' '
            ...
            if (result == 0) {
                ...
                chunk.offset = chunkIndex * pBook->CHUNK_SIZE;
                chunk.size = qMin(pBook->CHUNK_SIZE,
                                  pBook->m_totalFileSize - chunk.offset);
                chunk.uploaded = false;
                pBook->m_uploadQueue.enqueue(chunk);
...
void TcpClient::on_pwd_le_returnPressed() { on_login_pb_clicked(); }
